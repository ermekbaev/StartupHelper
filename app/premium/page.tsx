...
const faqs = [
  {
    ...
    answer:
      "Да, вы можете перейти на другой тариф в любое время. При переходе на более дорогой тариф изменения вступят в силу немедленно. При переходе на более дешёвый — со следующего расчётного периода.",
  },
  {
    question: "Какие способы оплаты доступны?",
    answer:
      "Мы принимаем банковские карты (Visa, Mastercard, МИР), электронные кошельки, а также оплату по счёту для юридических лиц.",
  },
  {
    question: "Есть ли пробный период?",
    answer:
      "Да, для тарифов Pro и Business доступен 14-дневный бесплатный пробный период. Карта не требуется для активации.",
  },
  {
    question: "Что произойдёт с моими данными при отмене подписки?",
    answer:
      "Ваши данные сохранятся, но доступ к премиум-функциям будет ограничен. Вы всегда сможете вернуться к платному тарифу и продолжить работу.",
  },
  {
    question: "Есть ли скидки при годовой оплате?",
    answer:
      "Да! При оплате за год вы получаете скидку 20%. Это 2 месяца бесплатно!",
  },
];
      ...
      <section className="py-12 sm:py-16 lg:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex items-center justify-center space-x-4 mb-10 sm:mb-12">
            <span
              className={`text-sm sm:text-base ${!isAnnual ? "text-gray-900 font-semibold" : "text-gray-500"}`}
            >
              Ежемесячно
            </span>
            <button
            ...
            >
              <span
                className={`absolute left-0 top-0.5 w-6 h-6 bg-white rounded-full shadow-md transition-transform ${
            ...
            >
              Ежегодно
              <span className="ml-2 text-green-600 text-xs sm:text-sm font-medium">
                -20%
              </span>
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 max-w-6xl mx-auto">
            {pricingTiers.map((tier) => (
              ...
              >
                ...
                <div
                  className={`p-6 sm:p-8 ${tier.popular ? "pt-12 sm:pt-14" : ""}`}
                >
                  ...
                  <ul className="space-y-3">
                    ...
                    {tier.notIncluded?.map((feature, idx) => (
                      <li
                        key={`not-${idx}`}
                        className="flex items-start text-sm sm:text-base text-gray-400"
                      >
            ...
            <table className="w-full min-w-[600px]">
              <thead>
                <tr className="border-b-2 border-gray-200">
                  ...
                  <th className="text-center py-4 px-4 text-purple-600 font-semibold bg-purple-50 rounded-t-lg">
                    Pro
                  </th>
                  <th className="text-center py-4 px-4 text-gray-900 font-semibold">
                    Business
